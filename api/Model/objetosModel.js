...
async function agregarObjeto(nombre, mitologia, descripcion, portador_famoso, poder_principal) {
    const [result] = await connection.query(
        `INSERT INTO objetos_memorables (nombre, mitologia, descripcion, portador_famoso, poder_principal)
         VALUES (?, ?, ?, ?, ?)`,
...
async function modificarObjeto(id, nombre, mitologia, descripcion, portador_famoso, poder_principal) {
    const [result] = await connection.query(
        `UPDATE objetos_memorables SET nombre = ?, mitologia = ?, descripcion = ?, portador_famoso = ?, poder_principal = ?
         WHERE id_objeto = ?`,
